...
LATEST=$(curl -s https://api.github.com/repos/HikariKnight/ScopeBuddy/commits/main | jq -r '.sha')
...
# Get new hash
HASH=$(nix-prefetch-github HikariKnight ScopeBuddy --rev "$LATEST" 2>/dev/null | jq -r '.hash')
DATE=$(date +%Y-%m-%d)
